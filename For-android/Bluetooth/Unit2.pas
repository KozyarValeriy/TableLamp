...
type
  ...
  TForm1 = class(TForm)
    ...
    Button1: TButton;
    Button17: TButton;//кнопка получения списка устройств
...
procedure send(Send_data: string; m: byte);
    begin
        Try
            ...
            Form1.label2.Text := 'Отправлено: ' + Send_data;
            a.SendData(Send_data);
        Except
            ShowMessage('Неизвестная ошибка. Вероятно, нет подключения к модулю.');
        end;
    end;
...
procedure TForm1.TrackBar1Change(Sender: TObject);
    begin
        timer1.Enabled := false;
        label5.Text := 'Залить цветом: ' + floattostr(roundto(TrackBar1.Value, 0));
...
procedure TForm1.TrackBar2Change(Sender: TObject);
    begin
        ...
        label4.Text := 'Скорость эффектов: ' + floattostr(roundto(TrackBar2.Value, 0));
        mode := 3;
        timer3.Enabled := true;
    end;
